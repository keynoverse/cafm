<div class="container-fluid">
    ...
    <div class="card">
        <div class="card-body">
            <form method="POST" action="<?= $this->url("floors/{$floor['id']}/edit") ?>" data-validate>
                <?= $this->csrf_field() ?>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" class="form-control <?= $this->hasError('name') ? 'is-invalid' : '' ?>" 
                                value="<?= $this->old('name', $floor['name']) ?>" required>
                            ...
                            <select name="building_id" class="form-select <?= $this->hasError('building_id') ? 'is-invalid' : '' ?>" required>
                                ...
                                <?php foreach ($buildings as $building): ?>
                                    <option value="<?= $building['id'] ?>" 
                                        <?= $this->old('building_id', $floor['building_id']) == $building['id'] ? 'selected' : '' ?>>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status" class="form-select <?= $this->hasError('status') ? 'is-invalid' : '' ?>" required>
                                <option value="active" <?= $this->old('status', $floor['status']) === 'active' ? 'selected' : '' ?>>Active</option>
                                <option value="inactive" <?= $this->old('status', $floor['status']) === 'inactive' ? 'selected' : '' ?>>Inactive</option>
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" class="form-control <?= $this->hasError('description') ? 'is-invalid' : '' ?>" 
                                rows="3"><?= $this->old('description', $floor['description']) ?></textarea>
                ...
                <div class="text-end mt-4">
                    <button type="submit" class="btn btn-primary">
                        <i class='bx bx-save'></i> Update Floor
                    </button>
                </div>
            </form>
        </div>
