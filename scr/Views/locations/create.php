<div class="container-fluid">
    <div class="row mb-4">
        <div class="col">
            <h1 class="h3">Create Location</h1>
        </div>
    ...
    <div class="card">
        <div class="card-body">
            <form method="POST" action="<?= $this->url('locations/create') ?>" data-validate>
                <?= $this->csrf_field() ?>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="room_id" class="form-select <?= $this->hasError('room_id') ? 'is-invalid' : '' ?>" required>
                                ...
                                <?php foreach ($rooms as $room): ?>
                                    <option value="<?= $room['id'] ?>" <?= $this->old('room_id') == $room['id'] ? 'selected' : '' ?>>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="type" class="form-select <?= $this->hasError('type') ? 'is-invalid' : '' ?>" required>
                                ...
                                <option value="desk" <?= $this->old('type') === 'desk' ? 'selected' : '' ?>>Desk</option>
                                <option value="cabinet" <?= $this->old('type') === 'cabinet' ? 'selected' : '' ?>>Cabinet</option>
                                <option value="storage" <?= $this->old('type') === 'storage' ? 'selected' : '' ?>>Storage</option>
                ...
                <div class="text-end mt-4">
                    <button type="submit" class="btn btn-primary">
                        <i class='bx bx-save'></i> Create Location
                    </button>
                </div>
            </form>
        </div>
